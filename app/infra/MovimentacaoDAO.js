...
MovimentacaoDAO.prototype.lista = function( predio, callback) {
    this._app.infra.connectionFactory(function(err, connection) {    
    	var sql = 'select m.movimentoId, m.descricao, m.tipoRegistro, m.valor, m.dataInserido, m.loginId, m.referencia, m.predio from movimentacao m where m.predio = ? ';
...
MovimentacaoDAO.prototype.salvaLista = function(contas, callback) {
    this._app.infra.connectionFactory(function(err, connection) { 
        ...
        for (var i = contas.length - 1; i >= 0; i--) {
            var dataInserido = moment(contas[i].dataPagamento).format('YYYY-MM-DD HH:mm:ss');
            var descricao = '';
            var referencia = moment(contas[i].dataPagamento).set('date', 2).format('YYYY-MM-DD HH:mm:ss');
...
MovimentacaoDAO.prototype.listaReferencia = function(predio, callback) {
    this._app.infra.connectionFactory(function(err, connection) { 
    	var sql = 'select r.referenciaId, r.referencia, r.predio_id from referencia r where r.predio_id = ? ';
        connection.query(sql, [predio], function(erros, results) {
            connection.release();
            callback(erros,results);
        });
